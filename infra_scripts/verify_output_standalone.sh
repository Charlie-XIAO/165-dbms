...
# This script verifies the output of a test case against the expected output.
...
function verify_output {
  ...
  # Remove all commands up to calling the client, strip comments (-- to eol),
  # remove trailing and leading whitespace and remove blank lines; also the last
  # awk is making sure all decimals have 2 rounded digits
  sed -r 's/\x1B\[([0-9]{1,2}(;[0-9]{1,2})?)?[m|K]//g' "${CUR_OUT_FILE}" | \
    awk '{sub("--.*$","",$0);print;}' | \
